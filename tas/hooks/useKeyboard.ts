...
const logger = createLogger("useKeyboard")
...
/**
 * Custom hook for handling keyboard input in the TabSwitcher
 *
 * Uses shared key bindings from tas/keyboard for consistency with native app.
 ...
 * useKeyboard({
 *   enabled: isVisible,
 *   keyboard: { modifier: 'Alt', activateForward: 'Tab', ... },
...
export function useKeyboard({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const bindings = getKeyBindings(keyboard)
      const primaryModifierPressed = isModifierPressed(keyboard.modifier, e.altKey, e.metaKey, e.ctrlKey, e.shiftKey)
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      if (isModifierRelease(e.key, keyboard.modifier)) {
        onActivateSelected()
      }
    }
